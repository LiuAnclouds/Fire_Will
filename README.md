...
NPC 点和技能目标点使用窗口自适应坐标。录入后，移动窗口、任意改变窗口宽高或恢复原尺寸，
都会按当前 Warcraft III 客户区实时换算，不需要再次录入。流程需要的点位未记录完整时，
程序会在执行前列出缺少项并停止，避免误点。
